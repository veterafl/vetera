...
only from the official FL DOH record, and only for dentists FL DOH files under a
Florida county (data/raw/doh-city.json, built by harvest_city_doh.py), with
hand-verified overrides on top. Records with no match are left without a city —
the UI omits the city line, never guesses and never mislabels an out-of-state
address as ", FL".
...
for r in dent:
    ...
    key = digits(r["lic"])
    city = overrides.get(key) or doh_city.get(key)
